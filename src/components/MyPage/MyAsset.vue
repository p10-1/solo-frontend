<template>
  <div class="asset-manager container mt-5">
    <h2 class="title">내 자산 관리</h2>
    <div class="button-box">
      <button class="button-main btn btn-primary" v-if="!loaded" @click="getAsset">
        ...
        <ul class="list-group">
          ...
          >
            <div v-if="!editMode">
              ...
              {{ asset.amount ? asset.amount.toLocaleString() : '0' }}원
            </div>
            <div v-else>
  ...
  methods: {
    async getAsset() {
      try {
        ...
        // 보험 데이터를 수동으로 처리
        if (
          response.data.insuranceCompany &&
          response.data.insuranceName &&
          response.data.insurance
        ) {
          this.assetTypes.insurance = JSON.parse(response.data.insuranceCompany).map(
            (company, index) => ({
              bank: company,
    ...
    prepareUpdatedData() {
      ...
      this.assetTypesList.forEach((type) => {
        ...
        updatedData[`${type}Account`] = JSON.stringify(
          this.assetTypes[type].map((asset) => asset.accountNumber)
        )
        updatedData[`${type}Bank`] = JSON.stringify(
          this.assetTypes[type].map((asset) => asset.bank)
        )
      })
      ...
      if (this.assetTypes.insurance.length > 0) {
        updatedData.insuranceCompany = JSON.stringify(
          this.assetTypes.insurance.map((ins) => ins.bank)
        )
        updatedData.insuranceName = JSON.stringify(
          this.assetTypes.insurance.map((ins) => ins.accountNumber)
        )
